  ...
  try {
    ...
    for (const [assetId, title, summary, outcome] of records) {
      const existing = await dbClient.query<{ id: string }>(
        `SELECT id FROM repair_memories WHERE organization_id = $1 AND asset_id = $2 AND title = $3 LIMIT 1`,
    ...
    console.log(`Seed complete: inserted=${inserted} skipped=${skipped} organization=${organizationId}`);
  } finally {
    dbClient.release();
    await pool.end();
  }
