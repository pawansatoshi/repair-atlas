...
-- CockroachDB vector indexing is production-ready in current releases. The embedding dimension must match the configured embedding model.
CREATE VECTOR INDEX IF NOT EXISTS repair_memories_embedding_idx ON repair_memories (organization_id, embedding) USING COSINE;
